...
def get_user_identifier(user_id: int, username: str = None) -> str:
    """Returns formatted user identifier with username and ID"""
    if username:
...
def handle_callback(update: Update, context: CallbackContext):
    ...
    # Payment approvals - FIXED
    if action.startswith("approve_"):
        if is_admin(user_id):
            session_id = action.replace("approve_", "")
            # Don't remove session immediately, process it
    ...
    if action.startswith("credit_"):
        ...
        if plan_key in CREDIT_PLANS:
            ...
            # Notify admin with user identifier
...
def process_payment_approval(update: Update, context: CallbackContext, session_id: str, approved: bool):
    ...
    if session_id not in pending_sessions:
        query.message.reply_text("⚠️ Session not found. The user may have already been processed or session expired.")
        return
        ...
        if session['type'] == 'credit':
            ...
            if activate_unlimited_plan(target_user, hours):
                try:
                    context.bot.send_message(
                        target_user,
                        f"✅ *Unlimited Plan Activated!*\n\n🌟 {duration_text} unlimited plan added!\n⏰ Expires in {duration_text}\n\nUse 📞 Number Lookup anytime without credit deduction!"
                    )
                except Exception as e:
                    logger.error(f"Failed to notify user: {e}")
                
                query.message.edit_text(f"✅ Unlimited plan activated for user {target_user} for {duration_text}!")
            else:
