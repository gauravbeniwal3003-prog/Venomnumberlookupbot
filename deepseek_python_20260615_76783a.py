...
UNLIMITED_PLANS = {
    "6h": {"duration_hours": 6, "price": 19, "label": "💰 ₹19 → 6 Hours Unlimited"},
    "12h": {"duration_hours": 12, "price": 39, "label": "💰 ₹39 → 12 Hours Unlimited"},
    "1d": {"duration_hours": 24, "price": 69, "label": "💰 ₹69 → 1 Day Unlimited"},
    "3d": {"duration_hours": 72, "price": 149, "label": "💰 ₹149 → 3 Days Unlimited"},
    "7d": {"duration_hours": 168, "price": 299, "label": "💰 ₹299 → 7 Days Unlimited"},
    "15d": {"duration_hours": 360, "price": 599, "label": "💰 ₹599 → 15 Days Unlimited"},
}
...
def handle_callback(update: Update, context: CallbackContext):
    ...
    if action.startswith("reply_"):
        if is_admin(user_id):
            ...
            query.message.reply_text(f"💬 Replying to user {target_user_id}\n\nSend your reply message:")
...
def process_payment_approval(update: Update, context: CallbackContext, session_id: str, approved: bool):
    ...
    if session_id not in pending_sessions:
        query.message.reply_text("❌ Session expired.")
        return
    ...
    if approved:
        if session['type'] == 'credit':
            if modify_credits(target_user, session['credits']):
                try:
                    context.bot.send_message(
                        target_user,
                        f"✅ *Payment Approved!*\n\n{session['credits']} credits added to your account!\n\nUse 📞 Number Lookup to start."
                    )
                except:
                    pass
                query.message.reply_text(f"✅ Approved! {session['credits']} credits added.")
            else:
                query.message.reply_text("❌ Database error.")
        else:
            hours = session['duration_hours']
            days = hours // 24 if hours >= 24 else 0
            if days > 0:
                duration_text = f"{days} days" if days > 1 else f"{days} day"
            else:
                duration_text = f"{hours} hours"
            
            if activate_unlimited_plan(target_user, hours):
                try:
                    context.bot.send_message(
                        target_user,
                        f"✅ *Unlimited Plan Activated!*\n\n🌟 {duration_text} unlimited plan added!\n\nUse 📞 Number Lookup anytime."
                    )
                except:
                    pass
                query.message.reply_text(f"✅ Unlimited plan activated for {duration_text}!")
            else:
                query.message.reply_text("❌ Database error.")
    else:
        try:
            context.bot.send_message(target_user, "❌ Payment declined. Contact support.")
        except:
            pass
        query.message.reply_text("❌ Payment declined.")
    ...
    if state == 'search_user':
        ...
        try:
            if text.startswith('@'):
                ...
                res = supabase.table("users").select("*").eq("telegram_id", int(text)).execute()
...
🚫 Banned: {banned_status}
        ...
        try:
            ...
            if modify_credits(user_id, amount):
                update.message.reply_text(f"✅ {'Added' if amount > 0 else 'Deducted'} {abs(amount)} credits for {user_id}")
                try:
                    if amount > 0:
