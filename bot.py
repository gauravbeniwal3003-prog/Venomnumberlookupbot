...
# Fixed Syntax Error: Bracket close check absolute path
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if "Number Lookup" in text or "📞" in text:
        profile = sync_account(user_id)
        if not is_admin(user_id) and profile.get('credits', 0) < CREDIT_COST_PER_LOOKUP:
        ...
        credits_display = "♾️ Unlimited (Root Administrator)" if is_admin(user_id) else f"`{profile['credits']}`"
        status_msg = f"""
        ...
        if response.status_code == 200:
            ...
            if payload.get('status') == 'success' and payload.get('success'):
                ...
                if total_found > 0:
                    ...
                    cost_msg = "0 Credits (Admin Session)" if is_admin(user_id) else f"{CREDIT_COST_PER_LOOKUP} Credits Deducted"
