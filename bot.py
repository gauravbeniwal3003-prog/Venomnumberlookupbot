...
def start_health_server():
    ...
    # Render provides the port in the PORT environment variable. Default to 8080 if not set.
    port = int(os.environ.get("PORT", 8080))
    
    # Allow address reuse to avoid "Address already in use" errors on restarts
...
def handle_message(update: Update, context: CallbackContext):
    ...
    # Fix: Redirect to admin input handler if an admin action is currently pending
...
def main():
    # Render requirements compliance: Start dummy port listener thread
    ...
    # Corrected order: Handle text messages through unified conditions
