...
def start_health_server():
    ...
    with socketserver.TCPServer(("", port), HealthCheckHandler) as httpd:
        logger.info(f"Health connection container running on port: {port}")
        httpd.serve_forever()
...
def modify_credits(user_id: int, volume: int) -> bool:
    try:
        ...
        logger.error(f"Database balance sync error: {e}")
        return False
...
# ============= CONSOLE MESSAGE EXECUTION FLOWS =============
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if context.user_data.get('payment_state_token'):
        update.message.reply_text("⚠️ *Awaiting Payment Verification:* Please upload the transaction screenshot image directly here to proceed.", parse_mode='Markdown')
        return
    ...
    # Flexible matching for buttons (handles with/without exact emojis)
    if "Number Lookup" in text or "📞" in text and "Lookup" in text:
        profile = sync_account(user_id)
...
def execute_number_lookup(update: Update, context: CallbackContext, query_number: str):
    ...
    try:
        ...
        logger.error(f"Lookup failure error stack trace: {error}")
...
def process_admin_payment_verdict(update: Update, context: CallbackContext, session_id: str, verified: bool):
    ...
    if verified:
        if modify_credits(target_client, credit_payload):
            ...
            query.message.reply_text("❌ *Database Fault:* Unable to save updated data indexes.")
    else:
